...
% Speicherort: 
% MATLAB Code für den Prüfgerät (Arduino Nano33BLE)
...
String array_prognose[anzahl_Pin] = {                           //Array für die Prognoseausgaben
  ...
  "A2", "A3", "A4", "A5", "A6", "A7", "Tx", "Rx"                //Bit von Rx wird in der niedrigsten Bitstelle der result
};                                                                                                                       

unsigned char input, input_help;                                //Eingabebuchstabe zum Start initialisieren
unsigned char val = '1';                                        //Neutrale Daten von Prüflung/Prüfgerät zur Prüfung Tx/Rx initialisieren                                                 
...
void list();                                                    //Ausgabefunktion für List der Menü definieren
void help_list ();                                              //Ausgabefunktion für List der Help definieren
...
u_int32_t testTx (BLECharacteristic characteristic, u_int32_t result);    //Tx-Testfunktion definieren
u_int32_t testRx (BLECharacteristic characteristic, u_int32_t result);    //Rx-Testfunktion definieren
...
void setup() {
  ...
  BLE.setLocalName("Nano33BLEMASTER");                                      //Name der Prüfgerät umbenennen
    ...
    if (Serial.available()){
      ...
      switch (input) {
        ...
        case 'h': 
          ...
          Serial.print("w: Zur weittere Fragen");
          Serial.print("\n");
        break;
          
        case 'w':
          help_list ();                                                                        //Aufruf die Help_list
        break;
        ...
        //Case 1-4 gehören zur der Help_list
        case '1':
          Serial.println ("https://docs.arduino.cc/static/f08c7221ed8baabdbbf43b7f7ffd1604/ABX00030-datasheet.pdf");
          Serial.print("\n");
        break;
        
        case '2':
          Serial.print("Arduino Nano33BLE hat nur 2 Serial Pin Serial0 und Serial1. Die Serial0 dient fuer die Komunikation zwischen Host und Arduino. ");
          Serial.print("Wenn man mit Serial.print() irgendwas im Terminal ausgibt und sieht nirgendwas, dann diese Schnittstelle wahrscheinlich kaputt. ");
          Serial.print("Sie Seria1 dient fuer die Kommunikation zwischen 2 Arduinos. Wenn die Rx/Tx mit der Pruefssoftware defekt geprueft wird, koennte ");
          Serial.println("es nicht sein, dass diese Schnittstelle kaputt ist. Umgekehrt wird die Serial1 normal, wenn die Rx/Tx auch in Ordnung geprueft wird.");
          Serial.print("\n");
        break;
        
        case '3':
          Serial.print("Der Testprogrammdauer haengt davon ab, wie schnell die Bluetooth Verbindung ist und wie viele Delay es in der Software gibt. ");
          Serial.print("Summe der Delaytime ist ungefaehrt 120ms.");
          Serial.print("\n");
        break;

        case '4':
          Serial.println("Die Bluetooth (BLE) ist die Voraussetzung fuer die Pruefung. Wenn die Pruefgeraet keine Pruefling gefunden hat, koennte es folgende Gruende dafuer liegen: ");
          Serial.println("# Es gibt keine Pruef-Software in der Pruefling.");
          Serial.println("# False Verbindung zwischen der Pruefling und der Pruefsgeraet.");
          Serial.println("# Die BLE Funktionalitaet der Pruefling ist defekt.");
          Serial.print("\n");
        break;
...
void help_list () {                                                                           //Hilffunktion zur Fragen

  Serial.println("1: Wo findet man Arduino Datensheet ?");
  Serial.println("2: Wie kann man wissen, ob Serial pin funktioniert ? ");
  Serial.println("3: Wie schnell laeuft das Test Programm ?");
  Serial.println("4: Was passiert, wenn die Bluetooth der Pruefling/Pruefgeraet nicht funktioniert ?");
  Serial.println("-------------------------------");
  Serial.print("\n");
}
...
void characteristic_Callback (BLECharacteristic characteristic) {       //neue Charaktereigenschaften erstellen
  ...
  result = testTx (characteristic, result);                  //Aufruf der Tx-Testfunktion
 
  result = testRx (characteristic, result);                  //Aufruf der Rx-Testfunktion
  ...
  if (anzahl_defekt == 0) {                                  //wenn keine Defekt
    ...
    Serial.println(result,BIN);                             //resultsausgabe
...
u_int32_t testTx (BLECharacteristic characteristic, u_int32_t result) {    //Testen Tx pin, ob die Prüfling Daten sendungsfähig ist
  
  result = result << 1;
  characteristic.writeValue((byte)0x29);
  delay(time1);
...
u_int32_t testRx (BLECharacteristic characteristic, u_int32_t result) {    //Testen Rx pin, ob Daten von Prüfling gelesen werden können
  
  result = result << 1;
  Serial1.write(val);
  delay(time1);
  characteristic.writeValue((byte)0x2a);
  delay(time1);
  if (Serial1.read() == val) {
